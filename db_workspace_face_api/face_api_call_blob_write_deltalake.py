...
stg_face_api_response = "dbfs:/user/hive/warehouse/sentiment_db_prod.db/stg_face_api_response" #table containing the Face API responses (Azure blob storage mounted to dbfs)
...
attributes = ["emotion", "glasses", "smile", "age", "gender", "facialHair"] #Attributes to be requested from Face API
include_id = True
...
#Iterating through image blobs in blob storage
#Sending them to Face API
#Saving the response to table
    for blob in blob_list:
        ...
        spark.read.json(sc.parallelize([json_response])).withColumn("snapshot_timestamp", lit(datetime.fromtimestamp(int(blob.name)))).withColumn("api_response_timestamp", lit(datetime.fromtimestamp(round(time())))) \
             .write.format("delta").mode("append").save(stg_face_api_response) #Reading the Face API response and saving it to stg_face_api_response table
        blob_client.delete_blob() #Deleting the image blob after it's been processed
